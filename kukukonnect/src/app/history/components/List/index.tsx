...
function getAverage(history: HistoryType[], selectedDate: Date | null) {
  ...
  if (selectedDate) {
    ...
    filtered = history.filter((item) => {
      const itemDateString = new Date(item.timestamp).toLocaleDateString(
        "en-US"
      );
...
export default function HistoryTable() {
  ...
  return (
    <div className="history-table-container bg-[#FFFFFF] rounded-xl shadow-lg w-full px-2 sm:px-4 md:px-8 ">
      {loading && (
      ...
      <div className="overflow-x-auto w-full ">
        <table className="w-full border-collapse table-auto text-sm sm:text-base md:text-lg">
          <thead>
            <tr className="bg-[#D2914A] text-white text-center text-sm sm:text-base md:text-lg ">
              <th className="p-2 sm:p-3 md:p-5 border">Date</th>
              <th className="p-2 sm:p-3 md:p-5 border">Temperature</th>
              <th className="p-2 sm:p-3 md:p-5 border">Humidity</th>
            </tr>
          </thead>
          <tbody>
            {currentAverages.length > 0
              ? currentAverages.map((item, index) => (
                  ...
                  >
                    <td className="p-2 sm:p-3 border border-[#08423615] text-[#0B2C27] text-center text-sm sm:text-base md:text-xl">
                      {item.date}
                    </td>
                    <td className="p-2 sm:p-3 border border-[#08423615] text-[#0B2C27] text-center text-sm sm:text-base md:text-xl">
